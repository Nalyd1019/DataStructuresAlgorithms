...
public class PathFinder<Node> {
    ...
    public Result searchUCS(Node start, Node goal) {
        ...
        while (!pqueue.isEmpty()){
            ...
            if (!visited.contains(entry.node)) {
                for (DirectedEdge edge : graph.outgoingEdges(entry.node)) {
                    double costToNext = entry.costToHere + edge.weight();
                    pqueue.add(new PQEntry((Node) edge.to(), costToNext, entry));
                }
            }
    ...
    public Result searchAstar(Node start, Node goal) {
         ...
         * Change below this comment  *
         ******************************/
        Queue<PQEntry> pqueue = new PriorityQueue<>(Comparator.comparingDouble((entry) -> entry.aStarCost));


        PQEntry entry = new PQEntry(start, 0, null,0);
        pqueue.add(entry);
        ...
        while (!pqueue.isEmpty()){
            ...
            if (!visited.contains(entry.node)) {
                for (DirectedEdge edge : graph.outgoingEdges(entry.node)) {
                    double aStarCost = entry.costToHere + graph.guessCost((Node)edge.to(), goal) + edge.weight();
                    double costToNext = entry.costToHere + edge.weight();
                    pqueue.add(new PQEntry((Node) edge.to(), costToNext, entry, aStarCost));
                }
            }
            visited.add(entry.node);
        }
    ...
    private class PQEntry {
        ...
        PQEntry(Node n, double c, PQEntry bp, double asc) {
            node = n;
            costToHere = c;
            backPointer = bp;
            aStarCost = asc;
